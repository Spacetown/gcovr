...
class DecisionParser:
    ...
    def _parse_one_line(self, lineno: int, code: str) -> None:
        ...
        if linecov and linecov.has_branches:
            branchecov_list = list(linecov.branches())
            if (
                _is_a_loop(code)
                or _is_a_oneline_branch(code)
                or (_is_a_closed_branch(code) and (len(branchecov_list) == 2))
            ):
                if len(branchecov_list) == 2:
                    ...
                    linecov.decision = DecisionCoverageConditional(
                        ...
                        count_true=branchecov_list[0].count,
                        count_false=branchecov_list[1].count,
                    )
                else:

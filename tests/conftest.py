...
_CC_HELP_OUTPUT = subprocess.check_output(  # nosec: B603
    [CC, "--help", "--verbose"], encoding="utf-8", shell=False, stderr=subprocess.STDOUT
)
_CC_VERSION_OUTPUT = subprocess.check_output(  # nosec: B603
    [CC, "--version"], encoding="utf-8", shell=False, stderr=subprocess.STDOUT
)
...
def pytest_report_header(config: pytest.Config) -> tuple[str, ...]:
    ...
    return (
        ...
        f"   {subprocess.check_output(['cmake', '--version'], shell=False).splitlines()[0].decode()}",  # nosec: B603, B607
        f"   {subprocess.check_output(['make', '--version'], shell=False).splitlines()[0].decode()}",  # nosec: B603, B607
        f"   ninja {subprocess.check_output(['ninja', '--version'], shell=False).strip().decode()}",  # nosec: B603, B607

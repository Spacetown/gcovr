...
nox.options.sessions = ["lint", "black", "doc", "tests(version='gcc-5')"]
...
def black(session):
    ...
    if session.posargs:
        session.run(sys.executable, "-m", "black", *session.posargs)
    else:
        session.run(
            sys.executable, "-m", "black", "--diff", "--check", *BLACK_CONFORM_FILES
        )
        session.run(sys.executable, "-m", "black", "--diff", *DEFAULT_TEST_DIRECTORIES)
